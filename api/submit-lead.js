...
//
// CRM_API_TOKEN is shared across every site in the portfolio and the CRM has
// no per-site field, so SITE_SOURCE_TAG is prepended into customer.notes on
// every submission — without it, this site's leads are indistinguishable
// from every other portfolio site's leads in the CRM.

const CRM_ENDPOINT = 'https://thequotemasters.com/crm_api/api.php?action=push_lead';
const MAX_FIELD_LENGTH = 2000;
const ZIP_DEFAULT = '55038'; // Hugo, MN
const ADDRESS_DEFAULT = 'Hugo, MN';
const SITE_SOURCE_TAG = 'Site: commercialcleanershugo.com';
...
// Minutes since midnight for each SCHEDULES id, from the CRM questionnaire
// reference. Ids 1-37 run 08:00am on a 15-minute grid, but 38 and 39 are
// 05:30pm and 05:45pm — there is no 05:15pm — so this cannot be computed
// from the id alone.
...
function buildWizardPayload(body) {
  ...
  return {
    value: {
      ...
      // Key is `num_of_quotes`, not the PDF's `number_of_quotes`.
...
module.exports = async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', 'https://commercialcleanershugo.com');
